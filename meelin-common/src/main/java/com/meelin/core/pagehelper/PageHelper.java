...
public class PageHelper implements Interceptor {
    ...
    public static long count(ISelect select) {
        ...
        select.doSelect();
        return page.getRecordsTotal();
    }

    /**
     * 开始分页
     *
     * @param pageNum  页码,传过来的参数并不是实际页码(起始条数),需要进过计算
     ...
     * 开始分页
     *
     * @param offset 页码,传过来的参数并不是实际页码(起始条数),需要进过计算
     * @param limit  每页显示数量
     */
    ...
    public static <E> Page<E> startPage(int pageNum, int pageSize, boolean count, Boolean reasonable, Boolean pageSizeZero) {
        //传过来的参数并不是实际页码,需要进过计算
//        PageHelper.startPage(model.getStart() / model.getLength() + 1, model.getLength());
    ...
    public void setProperties(Properties p) {
        ...
        if (StrUtils.isNotEmpty(runtimeDialect) && runtimeDialect.equalsIgnoreCase("TRUE")) {
            this.autoRuntimeDialect = true;
            this.autoDialect = false;
            this.properties = p;
        } else if (StrUtils.isEmpty(dialect)) {
            autoDialect = true;
            this.properties = p;
        } else {
            autoDialect = false;
    ...
    public void setSqlUtilConfig(SqlUtilConfig config) {
        ...
        } else if (StrUtils.isEmpty(config.getDialect())) {
            autoDialect = true;
            this.sqlUtilConfig = config;
        } else {
            autoDialect = false;
